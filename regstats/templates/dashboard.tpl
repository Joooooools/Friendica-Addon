<div id="adminpage">
	...
	<div class="regstats-card-grid">
		...
		<div class="regstats-card regstats-card--pending{{if $pending_count > 0}} regstats-card--alert{{/if}}" aria-label="{{$desc_reg_pending|escape:'html'}}">
			...
			<a href="/moderation/users/pending" class="regstats-card-link">
...
document.addEventListener('DOMContentLoaded', function() {
	function setupBreakdown(type, prefix) {
		var bars = document.querySelectorAll('.regstats-chart-bar-wrapper[data-type="' + type + '"]');
		bars.forEach(function(bar) {
			bar.addEventListener('click', function() {
				...
				if (titleEl) {
					var formatStr = "{{$lbl_details_for|escape:'javascript'}}";
					titleEl.textContent = formatStr.replace('%s', label);
				}
