...
class Exporter
{
		...
		try {
			...
			while ($item = DBA::fetch($res)) {
				...
				if (strpos($html_content, '<img') !== false) {
					if (preg_match('/href=["\']([^"\']+\/image\/[^"\']+)["\']/i', $html_content, $m)) {
						$gallery_url = $m[1];
					} elseif (preg_match('/src=["\']([^"\']+\/photo\/[^"\']+)["\']/i', $html_content, $m)) {
						$gallery_url = $m[1];
					}
					if ($gallery_url && strpos($gallery_url, 'http') !== 0 && strpos($gallery_url, '//') !== 0) {
						$gallery_url = rtrim((string) DI::baseUrl(), '/') . '/' . ltrim($gallery_url, '/');
					}
				}
	...
	{
		try {
			return new \DateTimeZone($tz_str ?: 'UTC');
		} catch (\Exception $e) {
	...
	private function getBase64Avatar(int $uid): string
	{
		foreach ([5, 4] as $scale) {
			...
			if (DBA::isResult($photo)) {
				...
				if (!empty($data)) {
					return 'data:' . $photo['type'] . ';base64,' . base64_encode($data);
				}
			}
		}
	...
	private function formatDate(\DateTime $date, bool $short = false): string
	{
		...
		if ($short) {
			return sprintf('%02d. %s %s', $day, $this->months[$month], $year);
		}

		if ($this->is_de) {
			return sprintf('%d. %s %s, %s Uhr', $day, $this->months[$month], $year, $time);
		}
	...
	private function filterHtml(string $html, string $privacy): string
	{
		...
		if ($privacy === 'placeholder' && strpos($html, '<img') !== false) {
			...
			$html = preg_replace_callback('/(<a\s+[^>]*href=["\']([^"\']+)["\'][^>]*>)?\s*(<img[^>]+src=["\']([^"\']+)["\'][^>]*>)\s*(<\/a>)?/i', function ($matches) use ($base_url) {
				...
				$alt = '';
				if (preg_match('/alt=["\']([^"\']+)["\']/i', $matches[3], $am)) {
					$alt = trim($am[1]);
				}
				if (empty($alt) && preg_match('/title=["\']([^"\']+)["\']/i', $matches[3], $tm)) {
					$alt = trim($tm[1]);
				}
		...
		if (strpos($html, '<a') !== false) {
			$html = preg_replace_callback('/<a\s+(.*?)href=["\'](.*?)["\'](.*?)>/i', function ($matches) {
				...
				if (strpos($url, 'http') === 0) {
					return '<a href="' . htmlspecialchars($url, ENT_QUOTES) . '" rel="noreferrer noopener nofollow" target="_blank">';
				}
				return $matches[0];
			}, $html);
		}
