...
echo "RealMember: Downloading latest spam domains...\n";
...
if ($rawContent === false) {
	realmember_fail("ERROR: Could not fetch list from GitHub. Please check your internet connection and timeout.\n");
}
...
foreach ($lines as $line) {
	...
	if (strlen($line) > 253 || !preg_match($domainRegex, $line)) {
		fwrite(STDERR, "WARNING: Skipping invalid line: " . substr($line, 0, 80) . "\n");
		$skipped++;
		continue;
	}
...
if ($count < REALMEMBER_MIN_DOMAINS) {
	realmember_fail(sprintf(
		"ERROR: Only received %d valid domains (Minimum amount: %d). Update aborted, existing list remains unchanged.\n",
		$count,
...
if ($skipped > 0) {
	echo "Notice: $skipped invalid lines were skipped.\n";
}

// Build the PHP output file content.
$output  = "<?php\n";
$output .= "/**\n";
$output .= " * Automatically generated by RealMember Updater on " . date('Y-m-d H:i:s') . "\n";
$output .= " * Source: $sourceUrl\n";
$output .= " * Amount of valid domains: $count\n";
$output .= " */\n\n";
$output .= "return [\n";
...
if (file_put_contents($tmpFile, $output) === false) {
	realmember_fail("ERROR: Could not write to temporary file $tmpFile. Please check write permissions.\n");
}

if (!rename($tmpFile, $targetFile)) {
	@unlink($tmpFile);
	realmember_fail("ERROR: Could not rename temporary file to $targetFile.\n");
}

// Update timestamp file (best-effort; failure here is non-fatal).
@file_put_contents($updateFile, date('Y-m-d H:i:s'));

echo "SUCCESS: $count domains successfully saved.\n";
